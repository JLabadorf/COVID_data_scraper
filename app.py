...
def county_data():
    ...
    df = pd.read_csv(oh_url)
    if param=="d":
        df_county = df[['County','Case Count','Death Count','Hospitalized Count','Onset Date']]
    else:
        df_county = df[['County','Case Count','Death Count','Hospitalized Count']]
    df_county['Case Count'] = df_county['Case Count'].str.replace(',','').astype(int)
    df_county['Death Count'] = df_county['Death Count'].str.replace(',','').astype(int)
    ...
    if param =="d":
        ...
        for i, rows in df_county.iterrows():
            ...
            hosp_count = rows[2]
            d[date][county] = []
            d[county].append({"Case Count":case_count,"Death Count":death_count,"Hospitalized Count":hosp_count})
